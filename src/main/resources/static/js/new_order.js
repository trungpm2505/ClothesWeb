...
//get data for table for first
		//vì đơn hàng chưa xác nhận nên status là 1
		//get data for table for first
		loadData(0,1,false,"",null,null,null);
		//get data for table
		function loadData(page,status,search,key,createAt,startDate,endDate) {
			var table = "#new-order-table tbody";
	...
	searchButton.addEventListener("click", function() {
	  ...
	  loadData(0,1,search,key,dateValue,startDateValue,endDateValue);
	});
	...
	function updateStatus(event){
		...
		$.ajax({
	      ...
	      success: function() {
	        ...
	        loadData(currentPageForEditAndDelete,1);
