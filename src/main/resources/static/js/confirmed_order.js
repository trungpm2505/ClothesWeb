...
//get data for table for first
		//vì đã xác nhận đơn hàng nên status là 3
		//get data for table for first
		loadData(0,3,false,"",null,null,null);
		//get data for table
		function loadData(page,status,search,key,createAt,startDate,endDate) {
			var table = "#confirmed-order-table tbody";
	...
	searchButton.addEventListener("click", function() {
	  ...
	  loadData(0,3,search,key,dateValue,startDateValue,endDateValue);
	});
	 ...
	 $('#refuseOrder').on('show.bs.modal', function (e) {
		...
		//$('#refuse-order-modal').attr({"data-id" : orderId,"data-update-status" : status});	    
	...
	function updateStatus(event){
		...
		$.ajax({
	      ...
	      success: function() {
	        ...
	        loadData(currentPageForEditAndDelete,3);
